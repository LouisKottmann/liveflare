...
def add_liveflare_options(opts)
  ...
  opts.on('-s', '--password [PASSWORD]', String, "Livebox's admin password " \
                                               + "(defaults to '#{$options[:password]}')") do |password|
    $options[:password] = password
  end
  opts.on('-i', '--interval [SECONDS]', Integer, "Time to wait between IP checks " \
                                     + "(defaults to #{$options[:interval]})") do |interval|
...
def check_liveflare_options(opts)
